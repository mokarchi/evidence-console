...
export function App() {
  ...
  const liveStatusCopy = liveData.status === "loading" ? "Connecting to API…" : liveData.status === "ready" ? `API connected · ${liveData.ingestion.assignments} assignments · ${liveData.ingestion.exposures} exposures · ${liveData.ingestion.outcomes} outcomes${liveData.analysis?.result?.survivalLtv ? " · Survival LTV ready" : ""}${liveData.segmentAnalysis?.ready ? ` · ${liveData.segmentAnalysis.testedSegments} subgroups` : ""}` : "API offline · showing seeded snapshot";
  const activeStageCopy = useMemo(() => `${stages.find((stage) => stage.id === activeStage)?.copy} · ${liveStatusCopy}`, [activeStage, liveStatusCopy]);

  useEffect(() => {
    let mounted = true;
    syncDemoExperiment()
      .then((result) => { if (mounted) setLiveData({ status: "ready", ingestion: result.ingestion, analysis: result.analysis, segmentAnalysis: result.segmentAnalysis, error: "" }); })

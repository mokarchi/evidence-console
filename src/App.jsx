...
function MetricContract({ expanded, setExpanded }) {
  return <section className={`panel contract-panel ${expanded ? "is-expanded" : ""}`}><div className="panel-heading"><div className="eyebrow-row"><h2>Metric Contract</h2><span className="version-badge">v1.1.0</span></div><button className="link-button">Version history</button></div><div className="contract-grid"><div className="contract-row"><strong>Schema</strong><span>evidence-console.metric-contract/v1</span><Copy size={13} /></div><div className="contract-row"><strong>Name</strong><span>90-Day Contribution LTV per User</span><Copy size={13} /></div><div className="contract-row"><strong>Unit</strong><span>USD</span><Copy size={13} /></div><div className="contract-row"><strong>Definition</strong><span>Total contribution margin attributable to a user within 90 days of first exposure.</span><Copy size={13} /></div><div className="contract-row"><strong>Numerator</strong><span>Σ (order_revenue − variable_cost − payment_fee − refund_amount)</span><Copy size={13} /></div><div className="contract-row"><strong>Denominator</strong><span># of exposed users</span><Copy size={13} /></div><div className="contract-row"><strong>Exposure Event</strong><span>checkout_view</span><Copy size={13} /></div>{expanded && <><div className="contract-row"><strong>Attribution Window</strong><span>90 days from first exposure</span><Copy size={13} /></div><div className="contract-row"><strong>Population</strong><span>Users with a first checkout_view exposure</span><Copy size={13} /></div><div className="contract-row"><strong>Filters</strong><span>country IN (US, CA, GB, AU) AND is_test_user = false</span><Copy size={13} /></div></>}<div className="contract-row contract-guardrails"><strong>Guardrails</strong><span><em>Min Sample Ratio (T/C): 0.80</em><em>Event Integrity Pass</em><em>Max SRM p-value: &gt; 0.01</em></span><Copy size={13} /></div><div className="contract-row"><strong>Updated</strong><span>Aug 18, 2026 by data-eng</span><Copy size={13} /></div></div><button className="contract-toggle" onClick={() => setExpanded(!expanded)}>{expanded ? "Show less" : "Show full contract"} {expanded ? <CaretUp size={15} /> : <CaretDown size={15} />}</button></section>;
}
...
export function App() {
  ...
  const [liveData, setLiveData] = useState({ status: "loading", ingestion: null, analysis: null, monitor: null, error: "" });
  const liveStatusCopy = liveData.status === "loading" ? "Connecting to API…" : liveData.status === "ready" ? `API connected · ${liveData.ingestion.assignments} assignments · ${liveData.ingestion.exposures} exposures · ${liveData.ingestion.outcomes} outcomes${liveData.analysis?.result?.survivalLtv ? " · Survival LTV ready" : ""}${liveData.segmentAnalysis?.ready ? ` · ${liveData.segmentAnalysis.testedSegments} subgroups` : ""}${liveData.monitor?.evaluation ? ` · Stop rule ${liveData.monitor.evaluation.status}` : ""}` : "API offline · showing seeded snapshot";
  const activeStageCopy = useMemo(() => `${stages.find((stage) => stage.id === activeStage)?.copy} · ${liveStatusCopy}`, [activeStage, liveStatusCopy]);

  useEffect(() => {
    let mounted = true;
    syncDemoExperiment()
      .then((result) => { if (mounted) setLiveData({ status: "ready", ingestion: result.ingestion, analysis: result.analysis, segmentAnalysis: result.segmentAnalysis, monitor: result.monitor, error: "" }); })
      .catch((error) => { if (mounted) setLiveData({ status: "offline", ingestion: null, analysis: null, monitor: null, error: error.message }); });
